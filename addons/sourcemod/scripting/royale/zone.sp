...
int CreateZoneProp(const float origin[3], const int color[4])
{
	...
	if (IsValidEntity(zone))
	{
		...
		SetEntityModel(zone, ZONE_MODEL);
...
Action Timer_StartDisplay(Handle timer)
{
...
void Zone_Think()
{
	...
	PrintToServer("%d", GetEntProp(g_ZonePropRef, Prop_Data, "m_iEFlags") & 1 << 17);
	...
	if (g_ZoneShrinkStart + duration > GetGameTime())
	{
		...
		ScaleVector(vecZoneOrigin, progress); //Scale by percentage
		AddVectors(vecZoneOrigin, g_ZonePropcenterOld, vecZoneOrigin); //Add distance to old center
		TeleportEntity(g_ZonePropRef, vecZoneOrigin, NULL_VECTOR, NULL_VECTOR);
	...
	for (int client = 1; client <= MaxClients; client++)
	{
		...
		float radius = g_ZoneConfig.diameter_max * percentage / 2.0;
		float ratio = GetVectorDistance(originClient, vecZoneOrigin) / radius;
		bool outsideZone = ratio > 1.0;
		
		//PrintToServer("%N outside zone? %f", client, ratio);
	}
